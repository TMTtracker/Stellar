...
const POST_COLUMNS =
  "id, user_id, post_title, post_body, post_tag, post_likes, post_time, profiles(display_name, xp_total), comments:post_comments(count)";
const POST_COLUMNS_WITH_IMAGE =
  "id, user_id, post_title, post_body, post_tag, post_likes, post_time, post_image_url, profiles(display_name, xp_total), comments:post_comments(count)";
...
export async function uploadPostImage(file) {
  ...
  try {
    ...
    if (
      uploadError &&
      !/Bucket not found|not found/i.test(uploadError.message ?? "")
    ) {
      // Non-bucket error: still fallback but surface warning
      console.warn(
        "[community] storage upload failed, using data URL fallback:",
        uploadError.message,
      );
    }
  } catch (e) {
    console.warn(
      "[community] storage upload exception, using data URL fallback:",
      e.message,
    );
  }
