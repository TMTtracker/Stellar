...
const friends = [
  { initials: "TM", name: "Tara M.", level: 12, xp: "8,450 XP", streak: "12-day streak", cls: "bg-[#A9D8AE]" },
  { initials: "FA", name: "Finn A.", level: 9, xp: "5,210 XP", streak: "7-day streak", cls: "bg-[#141814]" },
];

const moreFriends = [
  { initials: "JK", name: "Jules K.", level: 7, xp: "3,180 XP", streak: "5-day streak", cls: "bg-[#E8933E]" },
  { initials: "RS", name: "Rin S.", level: 6, xp: "2,640 XP", streak: "4-day streak", cls: "bg-[#6B5BA6]" },
  { initials: "LM", name: "Leo M.", level: 5, xp: "1,920 XP", streak: "3-day streak", cls: "bg-[#5B8FA6]" },
];
...
const BUILD_DISPLAY = {
  ...
  interview_prep_dojo: {
    icon: Warehouse,
    iconColor: "text-[#6A6F73] dark:text-[#8FA893]",
...
const MATERIAL_FIELDS = [
  {
    key: "bricks",
    required: "bricks_required",
    label: "Bricks",
    icon: BrickWall,
  },
  {
    key: "timber",
    required: "timber_required",
    label: "Timber",
    icon: TreeDeciduous,
  },
  {
    key: "rare_gem",
    required: "rare_gem_required",
    label: "Rare gem",
    icon: Gem,
  },
  ...
  {
    key: "crystal_shard",
    required: "crystal_shard_required",
    label: "Crystal shard",
    icon: Lightbulb,
  },
  {
    key: "fabric",
    required: "fabric_required",
    label: "Fabric",
    icon: Feather,
  },
];
...
export default function Dashboard() {
  ...
  const displayName =
    user?.user_metadata?.full_name || user?.email || "Stellar Cadet";
  const initials = displayName
    ...
    .slice(0, 2)
    .toUpperCase();
  const totalMaterialCount = MATERIAL_FIELDS.reduce(
    (sum, f) => sum + (resources[f.key] ?? 0),
    0,
  );
  ...
  const nextBaseLevelInfo =
    nextBaseLevelNum <= 6
      ? baseLevels.find((l) => l.level === nextBaseLevelNum)
      : null;
  const baseLevelMet = nextBaseLevelInfo
    ? level >= (nextBaseLevelInfo.unlock_level ?? 1)
    : false;
  const baseRequirements = nextBaseLevelInfo
    ? MATERIAL_FIELDS.map((f) => ({
        key: f.key,
        required: nextBaseLevelInfo[f.required] ?? 0,
        have: resources[f.key] ?? 0,
      })).filter((f) => f.required > 0)
    : [];
  const baseMaterialsHave = baseRequirements.reduce(
    (sum, f) => sum + Math.min(f.have, f.required),
    0,
  );
  const baseMaterialsNeeded = baseRequirements.reduce(
    (sum, f) => sum + f.required,
    0,
  );
  ...
  const baseReady =
    !!nextBaseLevelInfo &&
    baseLevelMet &&
    baseMaterialsHave >= baseMaterialsNeeded;
  ...
  useEffect(() => {
    ...
    listDailyLessons({ limit: 3 })
      ...
      .catch((e) => {
        if (!cancelled)
          setDailyError(e.message ?? "Failed to load daily lessons");
      })
      .finally(() => {
  ...
  async function handleUseBuilding(building) {
    ...
    try {
      await placeNewBuilding(building.build_id, {
        pos_x: 0,
        pos_y: 0,
        rotation: 0,
      });
    ...
    <ProtectedLayout>
      <div className="relative w-full h-screen overflow-hidden">
        ...
        <div className="absolute top-4 left-4 flex items-center gap-2">
          ...
          >
            ...
            <div>
              ...
              <div className="w-28 h-1.5 rounded-full bg-[#EAF2E6] dark:bg-[#232A24] mt-2 overflow-hidden">
                <div
                  className="h-full bg-[#A9D8AE] rounded-full"
                  style={{ width: `${pct}%` }}
                />
              </div>
            </div>
          ...
          >
            {nextIsMilestone ? (
              ...
              <>
                <Zap size={15} className="text-[#A9D8AE]" /> +{nextXp} XP
                tomorrow
              </>
            )}
          </div>
        ...
        {showDailyLessons && (
          <div className="absolute top-[84px] left-4 w-[280px] bg-white/95 dark:bg-[#14171A]/95 border border-[#C9DDC4] dark:border-[#262E28] rounded-xl px-4 py-4 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              <p className="text-[11px] font-bold tracking-wider text-[#6A6F73] dark:text-[#8FA893]">
                DAILY LESSONS
              </p>
              <button
            ...
            {dailyLoading && (
              <p className="text-[12px] text-[#6A6F73] dark:text-[#8FA893] py-3">
                Loading lessons…
              </p>
            )}
            {!dailyLoading && dailyError && (
              <p className="text-[12px] text-[#D9605B] py-3">{dailyError}</p>
            )}
                      ...
                      >
                        <div className="flex items-center justify-between mb-1">
                          <div className="min-w-0 pr-2">
                            <p className="text-[13px] font-medium text-[#1F2225] dark:text-[#F2F5F0] leading-tight truncate">
                              {lesson.title}
                            </p>
                            ...
                            {lesson.courses?.title && (
                              <p className="text-[11px] text-[#8BA089] mt-0.5 truncate">
                                {lesson.courses.title}
                              </p>
                            )}
                          </div>
                          <div className="flex items-center gap-2.5 text-[12px] shrink-0">
                            ...
                            <span className="flex items-center gap-1">
                              <MaterialIcon
                                size={14}
                                className="text-[#6A6F73] dark:text-[#8FA893]"
                              />
                              {lesson.material_qty}
                            </span>
                          </div>
            ...
            <button className="w-full" onClick={() => navigate("/courses")}>
              <p className="text-xs font-semibold tracking-wide text-center rounded-full text-white mt-3 p-3 border-t bg-[#8dc26d]">
                View all lessons
              </p>
            </button>
            <p className="text-[11px] text-[#6A6F73] dark:text-[#8FA893] mt-3 pt-3 border-t border-[#C9DDC4] dark:border-[#262E28]">
              Harder lessons drop rarer build materials, not just more XP.
            </p>
          </div>
        )}
        ...
        <div className="absolute top-4 right-4 flex items-center gap-2">
          {/* Friends stack - overlaps by default, spreads on hover so each icon is visible.
              Hovering one avatar shows that person's mini profile card. */}
          <div className="group flex items-center mr-1 px-1 py-1 rounded-full cursor-default" title="Friends online">
            {friends.map((f) => (
              <div key={f.initials} className="relative group/avatar">
                <div
                  className={`w-6.5 h-6.5 rounded-full ${f.cls} text-white flex items-center justify-center text-[10px] font-bold border-2 border-white dark:border-[#14171A] shadow-sm relative transition-all duration-200 -ml-2 first:ml-0 group-hover:ml-1.5 group-hover:first:ml-0 group-hover/avatar:scale-125 group-hover/avatar:-translate-y-0.5 group-hover/avatar:z-20 group-hover/avatar:border-[#A9D8AE] cursor-pointer`}
                >
                  {f.initials}
                </div>
                {/* Hover profile card */}
                <div className="pointer-events-none absolute top-[calc(100%+10px)] right-0 z-30 w-48 rounded-2xl border border-[#C9DDC4] dark:border-[#262E28] bg-white dark:bg-[#14171A] p-3 shadow-xl opacity-0 invisible translate-y-1 transition-all duration-200 group-hover/avatar:opacity-100 group-hover/avatar:visible group-hover/avatar:translate-y-0">
                  <div className="flex items-center gap-2.5">
                    <div className={`w-9 h-9 rounded-full ${f.cls} text-white flex items-center justify-center text-xs font-bold shrink-0`}>
                      {f.initials}
                    </div>
                    <div className="min-w-0">
                      <p className="text-sm font-bold text-[#1F2225] dark:text-[#F2F5F0] leading-tight truncate">{f.name}</p>
                      <p className="text-[11px] text-[#6A6F73] dark:text-[#8FA893]">Lv {f.level}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 mt-2.5 pt-2.5 border-t border-[#C9DDC4] dark:border-[#262E28] text-[11px] font-semibold text-[#6A6F73] dark:text-[#8FA893]">
                    <span className="flex items-center gap-1">
                      <Zap size={12} className="text-[#A9D8AE]" />
                      {f.xp}
                    </span>
                    <span className="flex items-center gap-1">
                      <Flame size={12} className="text-[#E8933E]" fill="currentColor" />
                      {f.streak}
                    </span>
                  </div>
                </div>
            ...
            <div className="relative group/more">
              <div
                className="w-6.5 h-6.5 rounded-full bg-white dark:bg-[#14171A] border-2 border-white dark:border-[#14171A] text-[#6A6F73] dark:text-[#8FA893] flex items-center justify-center text-[10px] font-bold -ml-2 group-hover:ml-1.5 shadow-sm relative transition-all duration-200 group-hover/more:scale-125 group-hover/more:-translate-y-0.5 group-hover/more:z-20 group-hover/more:border-[#A9D8AE] cursor-pointer"
              >
                +3
              </div>
              {/* Hover card listing the rest of the group */}
              <div className="pointer-events-none absolute top-[calc(100%+10px)] right-0 z-30 w-52 rounded-2xl border border-[#C9DDC4] dark:border-[#262E28] bg-white dark:bg-[#14171A] p-2 shadow-xl opacity-0 invisible translate-y-1 transition-all duration-200 group-hover/more:opacity-100 group-hover/more:visible group-hover/more:translate-y-0">
                <p className="text-[10px] font-bold tracking-wider text-[#6A6F73] dark:text-[#8FA893] px-2 pt-1 pb-2">3 MORE FRIENDS</p>
                {moreFriends.map((f) => (
                  <div key={f.initials} className="flex items-center gap-2.5 rounded-xl px-2 py-2 hover:bg-[#EFFAF4] dark:hover:bg-[#1E2B20]">
                    <div className={`w-8 h-8 rounded-full ${f.cls} text-white flex items-center justify-center text-[10px] font-bold shrink-0`}>
                      {f.initials}
                    </div>
                    <div className="min-w-0">
                      <p className="text-[13px] font-bold text-[#1F2225] dark:text-[#F2F5F0] leading-tight truncate">{f.name}</p>
                      <p className="text-[11px] text-[#6A6F73] dark:text-[#8FA893]">Lv {f.level} · {f.xp}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <button
            ...
            className="relative flex items-center gap-2 bg-white/95 dark:bg-[#14171A]/95 border border-[#C9DDC4] dark:border-[#262E28] rounded-xl px-3 py-1.5 hover:border-[#A9D8AE] transition-colors"
          >
          ...
          >
            ...
            <span className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-[#A9D8AE] text-white text-[10px] font-bold rounded-full flex items-center justify-center">
              3
            </span>
          </button>
          <button
            aria-label="Notifications"
            className="w-9 h-9 bg-white/95 dark:bg-[#14171A]/95 border border-[#C9DDC4] dark:border-[#262E28] rounded-xl flex items-center justify-center text-[#6A6F73] dark:text-[#8FA893] hover:border-[#A9D8AE] hover:text-[#A9D8AE] transition-colors"
          >
            <Bell size={16} />
          </button>
            ...
            {settingsOpen && (
              <div className="absolute right-0 top-11 z-20 w-52 rounded-2xl border border-[#C9DDC4] dark:border-[#262E28] bg-white dark:bg-[#14171A] p-2 text-[#1F2225] dark:text-[#F2F5F0] shadow-xl">
                ...
                >
                  ...
                  Night mode
                  <span
                    className={`ml-auto flex h-5 w-9 items-center rounded-full p-0.5 transition-colors ${dark ? "bg-[#A9D8AE]" : "bg-[#E4EBE2]"}`}
                  >
                    <span
                      className={`h-4 w-4 rounded-full bg-white shadow transition-transform ${dark ? "translate-x-4" : "translate-x-0"}`}
                    />
                  </span>
                </button>
                <button
        ...
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-1 bg-white/95 dark:bg-[#14171A]/95 border border-[#C9DDC4] dark:border-[#262E28] rounded-2xl px-2 py-1.5 shadow-sm">
          {nav.map(({ id, label, icon: Icon, to }) => (
            ...
            >
              <Icon
                size={18}
                className="text-[#6A6F73] dark:text-[#8FA893] hover:text-[#1F2225] dark:hover:text-[#EAF3E7]"
              />
              <span className="text-[11px] font-medium text-[#6A6F73] dark:text-[#8FA893]">
                {label}
              </span>
            </button>
          ))}
        ...
        {showInventory && (
          <>
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[520px] max-w-[92vw] max-h-[90vh] overflow-auto bg-white dark:bg-[#14171A] border border-[#C9DDC4] dark:border-[#262E28] rounded-2xl p-5 shadow-2xl z-10">
              ...
              <p className="text-[12px] text-[#6A6F73] dark:text-[#8FA893] mb-3">
                {coins.toLocaleString()} coins
              </p>

              <p className="text-[11px] font-bold tracking-wider text-[#6A6F73] dark:text-[#8FA893] mb-2">
                BUILD MATERIALS
              </p>
              <div className="grid grid-cols-4 gap-2 mb-4">
                {MATERIAL_FIELDS.map((f) => (
                  <div
                    key={f.key}
                    className="flex flex-col items-center justify-center rounded-xl bg-[#EEF6ED] dark:bg-[#1B211C] py-2.5"
                  >
                    <f.icon
                      size={20}
                      className="text-[#6A6F73] dark:text-[#8FA893]"
                    ...
                    <span className="text-[10px] text-[#6A6F73] dark:text-[#8FA893]">
                      {f.label}
                    </span>
                  </div>
                ))}
              ...
              <div className="space-y-2">
                {consumables.map((item) => {
                  ...
                  return (
                    <div
                      key={item.name}
                      className="flex items-center justify-between rounded-xl bg-[#EEF6ED] dark:bg-[#1B211C] px-3 py-2"
                    >
                      ...
                      <div className="flex items-center gap-2 text-[12px] text-[#6A6F73] dark:text-[#8FA893]">
                        ...
                        <button
                          ...
                          disabled={
                            usingBoost || boostActive || xpBoostCharges <= 0
                          }
        ...
        {showYourBuilds && (
          <>
            <div
              className="absolute inset-0 bg-black/20"
              onClick={() => setShowYourBuilds(false)}
            />
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[420px] max-w-[92vw] max-h-[80vh] overflow-auto bg-white dark:bg-[#14171A] border border-[#C9DDC4] dark:border-[#262E28] rounded-2xl p-5 shadow-2xl z-10">
              ...
              {unlockedBuildings.length === 0 && (
                <p className="text-[12px] text-[#6A6F73] dark:text-[#8FA893]">
                  Nothing unlocked yet - build one from the Build menu.
                </p>
              )}
              ...
              <div className="space-y-2">
                {unlockedBuildings.map((b) => {
                  const display =
                    BUILD_DISPLAY[b.build_id] ?? DEFAULT_BUILD_DISPLAY;
                  const Icon = display.icon;
                  return (
                    <div
                      key={b.build_id}
                      className="flex items-center justify-between rounded-xl bg-[#EEF6ED] dark:bg-[#1B211C] px-3 py-2.5"
                    >
        ...
        {showBuildMenuPanel && (
          <div className="absolute bottom-4 right-4 w-[520px] max-w-[92vw] max-h-[88vh] overflow-auto bg-white dark:bg-[#14171A] border border-[#C9DDC4] dark:border-[#262E28] rounded-2xl p-5 shadow-2xl z-20">
            ...
            <div className="grid grid-cols-2 gap-2.5">
              ...
              {buildMenu.length === 0 && (
                <p className="col-span-2 text-[12px] text-[#6A6F73] dark:text-[#8FA893]">
...
function BuildMenuCard({ build, resources, level, unlocked }) {
  ...
  const display = BUILD_DISPLAY[build.id] ?? {
    ...DEFAULT_BUILD_DISPLAY,
    desc: build.name,
  };
  const ItemIcon = display.icon;
  const levelMet = level >= (build.unlock_level ?? 1);

  const requirements = MATERIAL_FIELDS.map((f) => ({
    ...f,
    required: build[f.required] ?? 0,
    have: resources[f.key] ?? 0,
  })).filter((f) => f.required > 0);
  ...
  async function handleBuild() {
    ...
    try {
      ...
      setMessage(
        res?.ok
          ? "Crafted! Find it in Your Builds to place it on the map."
          : "You don't have enough materials for this.",
      );
    } catch (e) {
  ...
  async function handleBuyMissing() {
    ...
    try {
      ...
      setMessage(
        res?.ok
          ? "Materials purchased!"
          : `Not enough coins — you have ${res?.coins ?? 0}.`,
      );
    } catch (e) {
  ...
  return (
    <div className="rounded-xl border border-[#C9DDC4] dark:border-[#262E28] p-2.5">
      ...
      <p className="text-[12px] text-[#6A6F73] dark:text-[#8FA893] mb-2">
        {display.desc}
      </p>

      {!levelMet && (
        <div className="text-[12px] text-[#6A6F73] dark:text-[#8FA893]">
          Reach level {build.unlock_level} to unlock this building.
        </div>
      )}
      ...
      {levelMet && unlocked && (
        <div className="flex items-center gap-1.5 text-[12px] font-medium text-[#3E7A42] dark:text-[#8FE0A0] bg-[#E1F0DF] dark:bg-[#1B2B1D] rounded-lg px-2.5 py-2">
          <Check size={13} strokeWidth={3} /> Unlocked — place it from Your
          Builds.
        </div>
      )}
        ...
        <>
          {requirements.map((f) => {
            ...
            return (
              <div
                key={f.key}
                className="text-[12px] flex items-center justify-between mb-1.5"
              >
                <span className="flex items-center gap-1">
                  <MatIcon size={13} />
                  {f.label}
          ...
          {message && (
            <p className="text-[11px] text-[#6A6F73] dark:text-[#8FA893] mb-1.5">
              {message}
            </p>
          )}
          ...
          {allMet ? (
            ...
            >
              {busy
                ? "Buying…"
                : `Buy missing materials — ${build.coin_cost} coins`}
            </button>
          )}
        </>
      )}
