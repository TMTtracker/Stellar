...
export default function Communities() {
      ...
      channel = supabase
          ...
          (payload) => {
            if (payload.eventType === "UPDATE" && payload.new?.id) {
              ...
              setPosts((prev) =>
                prev.map((p) =>
                  p.id === payload.new.id
                    ? { ...p, post_likes: payload.new.post_likes }
                    : p,
                ),
              );
              ...
              setPosts((prev) =>
                prev.some((p) => p.id === payload.new.id)
                  ? prev
                  : [{ ...payload.new, comment_count: 0 }, ...prev],
              );
            }
          },
        )
        .on(
          ...
          (payload) => {
            ...
            if (
              payload.new?.id &&
              selfCountedCommentIds.current.has(payload.new.id)
            ) {
  ...
  const tagFiltered =
    activeTag === "All" ? threads : threads.filter((t) => t.tag === activeTag);
  ...
  async function handleToggleLike(thread) {
    const isReal =
      !!thread.id &&
      !String(thread.id).startsWith("placeholder") &&
      posts.some((p) => p.id === thread.id);
    ...
    if (!realId) {
      ...
      setLocalPlaceholderLikes((prev) => {
        ...
        return {
          ...prev,
          [key]: { likes: cur.likes + (nextLiked ? 1 : -1), liked: nextLiked },
        };
      });
      return;
    }
    ...
    setPosts((prev) =>
      prev.map((p) =>
        p.id === realId
          ? { ...p, post_likes: (p.post_likes ?? 0) + (wasLiked ? -1 : 1) }
          : p,
      ),
    );
    setLikedIds((prev) => {
      const next = new Set(prev);
      if (wasLiked) next.delete(realId);
    ...
    try {
      ...
      setLikedIds((prev) => {
        const next = new Set(prev);
        if (res.liked) next.add(realId);
        else next.delete(realId);
        return next;
      });
    } catch (e) {
      // Rollback on error
      ...
      if (next.has(thread.id)) {
        ...
        if (!commentsByPost[thread.id]) {
          setCommentsByPost((cur) => ({
            ...cur,
            [thread.id]: { loading: true, error: "", items: [] },
          }));
          listComments(thread.id)
            .then((items) => {
              setCommentsByPost((cur) => ({
                ...cur,
                [thread.id]: { loading: false, error: "", items },
              }));
    ...
    try {
      ...
      setCommentsByPost((cur) => ({
        ...
        [postId]: {
          loading: false,
          error: "",
          items: [...(cur[postId]?.items ?? []), created],
        },
      }));
  ...
  return (
    <ProtectedLayout>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <p className="text-[11px] font-bold tracking-wider text-[#A9D8AE]">
            CONNECT
          </p>
      ...
      <div className="grid grid-cols-[1fr_300px] gap-6">
        ...
        <div className="space-y-4">
          ...
          {filteredThreads.map((thread) => {
            ...
            const displayLikes = isReal
              ? thread.likes
              : (localLike?.likes ?? thread.likes);
            const isLiked = isReal
              ? likedIds.has(thread.id)
              : !!localLike?.liked;
            ...
            return (
              <article
                key={threadKey}
                className="bg-white dark:bg-[#14171A] rounded-2xl border border-[#C9DDC4] dark:border-[#262E28] p-5"
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-[#1F2225] dark:text-[#F2F5F0]">
                        {thread.author}
                      </p>
                ...
                <span className="inline-block text-[10px] font-bold tracking-wider text-[#A9D8AE] bg-[#DDF0E1] dark:bg-[#1E2B20] px-2.5 py-1 rounded-full mb-3">
                  {thread.tag}
                </span>

                <h2 className="font-bold leading-snug text-[#1F2225] dark:text-[#F2F5F0]">
                  {thread.title}
                </h2>
                <p className="text-sm text-[#6A6F73] dark:text-[#8FA893] mt-1.5 leading-relaxed">
                  {thread.body}
                </p>
                ...
                {commentsOpen && (
                  <div className="mt-4 pt-4 border-t border-[#EEF6ED] dark:border-[#262E28]">
                    ...
                    {commentState?.error && (
                      <p className="text-xs text-[#D9605B]">
                        {commentState.error}
                      </p>
                    )}
                    {!commentState?.loading &&
                    ...
                    {!commentState?.loading &&
                      commentState?.items.length > 0 && (
                        <div
                          className={`space-y-3 ${commentState.items.length > 3 ? "max-h-48 overflow-y-auto pr-1" : ""}`}
                        >
                            ...
                            >
                              ...
                              <div className="bg-[#F5FAF3] dark:bg-[#1B211C] rounded-xl px-3 py-2 flex-1">
                                <div className="flex items-center gap-2">
                                  <p className="text-xs font-bold text-[#1F2225] dark:text-[#F2F5F0]">
                                    {c.display_name}
                                  </p>
                                  <p className="text-[10px] text-[#8BA089]">
                                    {formatRelativeTime(c.created_at)}
                                  </p>
                                </div>
                                <p className="text-sm text-[#1F2225] dark:text-[#F2F5F0] mt-0.5 whitespace-pre-wrap break-words">
                                  {c.body}
                                </p>
                              </div>
                            </div>
        ...
        <aside className="space-y-6">
          <div className="bg-white dark:bg-[#14171A] rounded-2xl p-5 border border-[#C9DDC4] dark:border-[#262E28]">
            ...
            {leaderboardLoading && (
              <p className="text-xs text-[#6A6F73] dark:text-[#8FA893] py-4">
                Loading leaderboard…
              </p>
            )}
              ...
              leaderboardMembers.length > 0 && (
                <ul className="space-y-3">
                  {leaderboardMembers.map((m) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <span className="text-xs font-bold text-[#6A6F73] dark:text-[#8FA893] w-4">
                          {m.rank}
                        </span>
                        <div
                          className={`w-9 h-9 rounded-full ${m.color} text-white flex items-center justify-center font-bold text-sm`}
                        >
                          {m.name
                            .split(" ")
                            .map((w) => w[0])
                        ...
                        <div>
                          <p className="text-sm font-medium text-[#1F2225] dark:text-[#F2F5F0]">
                            {m.name}
                          </p>
                          <p className="text-xs text-[#6A6F73] dark:text-[#8FA893]">
                            Level {m.level}
                      ...
                      <span className="text-xs font-bold text-[#6A6F73] dark:text-[#8FA893]">
            ...
            <p className="text-[10px] text-[#8BA089] mt-3">
          ...
          <div className="bg-white dark:bg-[#14171A] rounded-2xl p-5 border border-[#C9DDC4] dark:border-[#262E28]">
            ...
            {onlineLoading && (
              <p className="text-xs text-[#6A6F73] dark:text-[#8FA893] py-2">
                Loading…
              </p>
            )}
            ...
            {!onlineLoading && !onlineError && (
              <>
                ...
                <p className="text-[10px] text-[#8BA089] mt-1">
                  Live data • updates automatically
                </p>
              </>
            )}
